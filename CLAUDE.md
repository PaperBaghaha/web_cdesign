...
`scripts/update-publications.ts` keeps `papers.bib` in sync. Two modes:

```bash
yarn pubs:import path/to/citations.bib   # merge a Google Scholar BibTeX export
yarn pubs:enrich                         # backfill metadata from OpenAlex + Crossref
yarn pubs:enrich --dry-run               # preview changes without writing
yarn pubs:enrich --limit=25              # process only the first 25 gaps
yarn pubs:enrich --overwrite             # let remote values replace existing ones
```

**Google Scholar has no public API** and serves a CAPTCHA to scrapers, so there is no
automated path from a Scholar profile. Use the supported export instead: open the
profile, tick the select-all checkbox, choose Export ▸ BibTeX, save the file, then
`yarn pubs:import <file>`. That BibTeX is generated by Scholar itself.

`pubs:enrich` fills whatever the export leaves thin (DOI, venue, abstract, pages,
volume) from OpenAlex, falling back to Crossref for ACM/IEEE proceedings. Both are
keyless and permit automation.

Merge safety:

- Entries are matched by **normalised title**, not citation key, so re-imports don't duplicate
- Curated fields (`selected`, `preview`, `video`, `abbr`, `award`, …) are never overwritten —
  see `CURATED_FIELDS` in the script
- Existing values win over remote ones unless `--overwrite` is passed
- A `papers.bib.<date>.bak` backup is written before any change

### Publication media

Publications carry media through two BibTeX fields, both resolved by `src/utils/media.ts`:

- `preview` — teaser image; bare filenames resolve against `site.publications.previewDir`
- `preview_alt` — alt text, also rendered as the figure caption
- `video` — YouTube, Vimeo, or a self-hosted file; bare filenames resolve against
  `site.publications.videoDir`

`resolveVideo()` classifies the URL: YouTube/Vimeo become a lazy 16:9 `<iframe>`
(via `youtube-nocookie`), playable extensions become a native `<video>` with the
preview image as poster, and anything unrecognised falls back to a plain link button.

`shi2026conceptvis` is wired up as the working reference for both.

### People profile pages

`src/pages/people/[slug].astro` generates one page per entry in the `people` collection.
Publications are matched against `papers.bib` by `src/utils/people.ts`:

- Exact normalised full-name match always wins
- A surname + first-initial match only applies when one side is abbreviated ("J. Shi"),
  which keeps lab members who share a surname apart
- Set `aliases: ['Maiden Name']` in the person's frontmatter for alternate spellings
- Set `noPage: true` to skip generating a profile page for someone
